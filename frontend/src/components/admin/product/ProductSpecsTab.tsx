...
export function ProductSpecsTab({ productId }: ProductSpecsTabProps) {
  ...
  const { data: productSpecs = [] } = useQuery(
    trpc.specs.getProductSpecs.queryOptions(productId)
  );

  const invalidateProductSpecs = async () => {
    await queryClient.invalidateQueries({
      queryKey: trpc.specs.getProductSpecs.queryKey(productId),
    });
  };
  ...
  const createSpecMutation = useMutation(
    trpc.specs.createProductSpec.mutationOptions({
      onSuccess: async () => {
  ...
  const removeSpecMutation = useMutation(
    trpc.specs.removeProductSpec.mutationOptions({
      onSuccess: async () => {
  ...
  const groupedSpecs = (productSpecs as ProductSpec[]).reduce(
    (acc: Record<string, ProductSpec[]>, spec) => {
  ...
  const selectedKeyData = (specKeys as SpecKey[]).find(
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="shadow-sm">
        <CardHeader className="border-b">
          <div className="flex items-center gap-2">
            ...
            <CardTitle>Thêm thông số kỹ thuật</CardTitle>
          </div>
          <CardDescription>
            Chọn nhóm, tên thông số và nhập giá trị
          </CardDescription>
        </CardHeader>
        <CardContent className="pt-6">
          <Form {...addSpecForm}>
      ...
      <Card className="shadow-sm">
        <CardHeader className="border-b">
          <CardTitle>Thông số kỹ thuật hiện tại</CardTitle>
          <CardDescription>
            Danh sách tất cả thông số kỹ thuật của sản phẩm
          </CardDescription>
        </CardHeader>
        <CardContent className="pt-6">
          {Object.keys(groupedSpecs).length > 0 ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-sm">
                Hãy thêm thông số kỹ thuật cho sản phẩm này
              </p>
            </div>
          )}
        </CardContent>
