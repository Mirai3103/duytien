          ...
          <table className="w-full">
            <thead className="border-b border-border">
              <tr className="text-left">
                <th className="px-6 py-4 text-sm font-medium text-muted-foreground">Khách hàng</th>
                <th className="px-6 py-4 text-sm font-medium text-muted-foreground">Liên hệ</th>
                <th className="px-6 py-4 text-sm font-medium text-muted-foreground">Đơn hàng</th>
                <th className="px-6 py-4 text-sm font-medium text-muted-foreground">Tổng chi tiêu</th>
                <th className="px-6 py-4 text-sm font-medium text-muted-foreground">Loại</th>
                <th className="px-6 py-4 text-sm font-medium text-muted-foreground">Trạng thái</th>
                <th className="px-6 py-4 text-sm font-medium text-muted-foreground">Ngày tham gia</th>
            ...
            <tbody>
              {customers.map((customer) => (
                <tr key={customer.id} className="border-b border-border hover:bg-secondary/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className="text-sm text-foreground">{customer.orders} đơn</span>
                  </td>
                  <td className="px-6 py-4">
                    <span className="font-semibold text-foreground">{customer.spent}</span>
                  </td>
                  <td className="px-6 py-4">
                    <Badge variant={typeConfig[customer.type as keyof typeof typeConfig].variant}>
                      {typeConfig[customer.type as keyof typeof typeConfig].label}
                    </Badge>
                  </td>
                  <td className="px-6 py-4">
