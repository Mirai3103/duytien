...
import { Star, Shield, Truck, RefreshCw, Heart, Share2, ChevronRight } from 'lucide-react'
  ...
  reviews: [
    {
      ...
      rating: 5,
      date: '2024-10-01',
      comment: 'Sản phẩm tuyệt vời! Hiệu năng mạnh mẽ, camera chụp ảnh đẹp. Rất hài lòng với lựa chọn của mình.',
      helpful: 45,
    },
    {
      id: 2,
      ...
      rating: 5,
      date: '2024-09-28',
      comment: 'iPhone 15 Pro Max xứng đáng là flagship 2024. Màn hình đẹp, pin trâu, chip A17 Pro quá mượt.',
      helpful: 32,
    },
    {
      id: 3,
      ...
      rating: 4,
      date: '2024-09-25',
      comment: 'Máy đẹp, chạy mượt. Tuy nhiên giá hơi cao. Nhưng chất lượng Apple thì không phải bàn.',
      helpful: 28,
    },
    {
      id: 4,
      ...
      rating: 5,
      date: '2024-09-20',
      comment: 'Camera quá đỉnh, zoom 5x rõ nét lắm. Chơi game cũng không bị nóng máy. 10 điểm!',
      helpful: 56,
    },
  ],
  ratingDistribution: {
...
function RouteComponent() {
  const { id: _id } = Route.useParams() // TODO: Use this to fetch actual product data
  const [selectedImage, setSelectedImage] = useState(0)
  const [selectedColor, setSelectedColor] = useState(mockProduct.colors[0].id)
  const [selectedStorage, setSelectedStorage] = useState(mockProduct.storage[0].id)

  const currentStorage = mockProduct.storage.find(s => s.id === selectedStorage)!
  const totalReviews = Object.values(mockProduct.ratingDistribution).reduce((a, b) => a + b, 0)
      ...
      <main className="flex-1">
        <div className="container mx-auto px-4 py-6">
          ...
          <div className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
            ...
            <span className="hover:text-primary cursor-pointer">Điện thoại</span>
            <ChevronRight className="h-4 w-4" />
            <span className="hover:text-primary cursor-pointer">{mockProduct.brand}</span>
            <ChevronRight className="h-4 w-4" />
            ...
            <div className="space-y-6">
                ...
                <CardContent className="p-6">
                  <div className="flex items-baseline gap-3 mb-2">
                    <span className="text-4xl font-bold text-primary">
                      {currentStorage.price.toLocaleString('vi-VN')}đ
                    </span>
                    {currentStorage.oldPrice && (
                      <span className="text-xl text-muted-foreground line-through">
                        {currentStorage.oldPrice.toLocaleString('vi-VN')}đ
                      </span>
                    )}
                  </div>
                  <p className="text-sm text-muted-foreground">
                    Tiết kiệm {(currentStorage.oldPrice - currentStorage.price).toLocaleString('vi-VN')}đ
                  </p>
                </CardContent>
              </Card>
              ...
              <div>
                <Label className="text-base font-semibold mb-3 block">
                  Chọn màu sắc: {mockProduct.colors.find(c => c.id === selectedColor)?.name}
                </Label>
                <div className="flex gap-2">
              ...
              {/* Storage Selection */}
              <div>
                <Label className="text-base font-semibold mb-3 block">Chọn dung lượng:</Label>
                <RadioGroup value={selectedStorage} onValueChange={setSelectedStorage}>
                  <div className="grid grid-cols-3 gap-3">
                    {mockProduct.storage.map((storage) => (
                      <div key={storage.id} className="relative">
                        <RadioGroupItem
                          value={storage.id}
                          id={storage.id}
                        ...
                        <Label
                          htmlFor={storage.id}
                          className="flex flex-col items-center justify-center p-4 border-2 rounded-lg cursor-pointer transition-all peer-data-[state=checked]:border-primary peer-data-[state=checked]:bg-primary/5 hover:border-muted-foreground"
                        >
            ...
            <CardContent className="p-6">
              <Tabs defaultValue="description" className="w-full">
                ...
                <TabsContent value="specifications" className="mt-6">
                  <div className="space-y-6">
                    {Object.entries(mockProduct.specifications).map(([category, specs]) => (
                      <div key={category}>
                          ...
                          <div className="space-y-2">
                            {[5, 4, 3, 2, 1].map((rating) => (
                              <div key={rating} className="flex items-center gap-2">
                                ...
                                <Progress
                                  value={
                                    (mockProduct.ratingDistribution[rating as keyof typeof mockProduct.ratingDistribution] /
                                      totalReviews) *
                                    100
                                  }
                                  className="flex-1"
