...
import { deleteFile, uploadBase64ToFile } from "@/utils/file.utils";
...
export const brandRoute = router({
	...
	create: publicProcedure
		...
		.mutation(async ({ input }) => {
			const fileName = uploadBase64ToFile(input.image!);
			const _brand = await db
				.insert(brandsTable)
				.values({ name: input.name, slug: input.slug, logo: fileName });
			return { success: true };
		}),
