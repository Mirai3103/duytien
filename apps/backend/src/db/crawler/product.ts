...
async function crawlProduct(page: number, limit: number) {
  ...
  return fetch(
    "https://papi.fptshop.com.vn/gw/v1/public/fulltext-search-service/category",
    {
      ...
      body: `{"skipCount":${skipCount},"maxResultCount":${limit},"sortMethod":"noi-bat","slug":"may-tinh-bang","categoryType":"category","location":{}}`,
      method: "POST",
      mode: "cors",
      credentials: "omit",
    }
...
const endPage = 3;
const limit = 16;
const parentId = await upsertCategory("Phụ kiện");
...
for (let page = startPage; page <= endPage; page++) {
  ...
  for await (const product of products) {
    try {
      ...
      const categoryId = await upsertCategory("Máy tính bảng", null);
