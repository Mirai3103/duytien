...
import db from "..";
import { products as productsTable, productVariants as productVariantsTable } from "../schema";
...
async function crawlProduct( product: any) {
  ...
  try {
    ...
    const slideImages = response.data.image.slideImages.map((image: any) => image.url);
    // Uncomment khi muốn lưu DB
    console.log(`${product.slug} ${product.id} ${product.metadata.slug}`)
    await db
      .update(productVariantsTable)
      .set({
        metadata: { ...product.metadata, specs: groups, image, slideImages, isProcessSpec: true },
      })
...
(async () => {
  const variants = await db.query.productVariants.findMany({
    where: sql`NOT (${productsTable.metadata} ? 'isProcessSpec')`,
  });
